...
contract SupplyChain {
  ...
  */
  ...
  modifier verifyCaller (address _address) {
    require (msg.sender == _address, "Message sender is not authorized");
    _;
  }
  ...
  modifier forSale(uint _sku) {
    require(items[_sku].seller != address(0)); // Ensure Item is initialized
  ...
  function buyItem(uint _sku)
    ...
    checkValue(_sku)
  {
    Item storage bought = items[_sku];
    // mutate state of bought item
    bought.state = State.Sold;
    bought.buyer = msg.sender;
    // transfer money to seller
    bought.seller.transfer(bought.price);
    emit LogSold(_sku);
  }
